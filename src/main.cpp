...
#define SERVICE_NAME             "Zeegine-Server" 
// Displayed name of the service 
#define SERVICE_DISPLAY_NAME     "Zeegine-Server Service" 
...
struct mg_connection *conn) {
	...
	if (event == MG_NEW_REQUEST) {
		if(strncmp(ri->uri,"/printer/",9)!=0) return zeegine::HandlePagerequest(conn);
		zeegine::HandleWebrequest(conn);
		// Mark as processed
		return (void*)"";
	} else {
		return NULL; //zeegine::HandlePagerequest(conn);;
	}
}
#if defined(__APPLE__) || defined(__linux)
...
int main(int argc, const char * argv[])
{
	...
	try {
        ...
        cerr << "ZeegineServer version " << Zeegine_SERVER_VERSION << endl;
		cerr << "error: Error parsing command line: " << ex.what() << endl;
	}
	po::notify(vm);

	if (vm.count("help")) {
        cout << "ZeegineServer version " << Zeegine_SERVER_VERSION << endl;
		cout << desc << "\n";
		return 1;
	}
	string confFile;
	...
	if(confFile.length() == 0) confFile = string("/etc/ZeegineServer.conf");
	...
	if(vm.count("install")) {
		...
		cout << "ZeegineServer service is installed." << endl;
Cleanup:
	...
	if(vm.count("remove")) {
		...
		{
			cout << "Stopping ZeegineServer service." << endl;
			Sleep(1000);
			...
			{
				cerr << "ZeegineServer service is stopped." << endl;
			}
			else
			{
				cerr << "ZeegineServer service failed to stop." << endl;
			}
		}
		...
		if (!DeleteService(schService))
			...
			cout << "ZeegineServer service is removed." << endl;
